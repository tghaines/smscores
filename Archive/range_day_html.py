...
/* Filter Bar */
.filter-bar { display:flex; flex-wrap:wrap; gap:12px; margin-bottom:20px; padding:16px; background:var(--bg2); border-radius:8px; border:1px solid var(--border); }
.filter-group { display:flex; flex-direction:column; gap:4px; }
.filter-group label { font-size:0.8rem; color:var(--text2); text-transform:uppercase; }
.filter-group input, .filter-group select { font-size:0.9rem; color:var(--text); background:var(--bg); border:1px solid var(--border); padding:8px 12px; border-radius:4px; min-width:140px; }
.filter-group input:focus, .filter-group select:focus { outline:none; border-color:var(--gold); }
.filter-clear { align-self:flex-end; background:transparent; border:1px solid var(--muted); color:var(--muted); padding:8px 16px; border-radius:4px; cursor:pointer; }
...
.match-name { color:var(--text2); font-size:0.85rem; }
...
<div class="container">
  ...
  <div class="filter-bar">
    <div class="filter-group">
      <label>Shooter</label>
      <input type="text" id="filterShooter" placeholder="Name..." oninput="applyFilters()">
    </div>
    <div class="filter-group">
      <label>Target</label>
      <input type="text" id="filterTarget" placeholder="Target..." oninput="applyFilters()">
    </div>
    <div class="filter-group">
  ...
  <table id="resultsTable">
    <thead>
      <tr>
        ...
        <th data-sort="match" onclick="sortTable('match')">Match</th>
    ...
    <tbody>
      ...
      <tr data-idx="{{ loop.index0 }}" data-target="{{ s.target }}" data-shooter="{{ s.shooter_name }}" data-match="{{ s.match_name }}">
        ...
        <td class="match-name">{{ s.match_name or '-' }}</td>
        <td>
...
function renderBadges(shots) {
  ...
  shots.forEach(function(s) {
    var sc = String(s.score).toUpperCase();
    if (s.isSighter) {
      html += '<span class="shot-badge sighter">S</span>';
    } else {
      var cls = 'score-' + sc;
...
stringData.forEach(function(s, i) {
  ...
  if (s.shots && s.shots.length) {
    ...
    // Store numeric score for sorting
...
stringData.forEach(function(s) { if (s.match) matchSet.add(s.match); });
var matchSelect = document.getElementById('filterMatch');
Array.from(matchSet).sort().forEach(function(m) {
...
function applyFilters() {
  ...
  var target = document.getElementById('filterTarget').value.toLowerCase();
  var match = document.getElementById('filterMatch').value;
  var rows = document.querySelectorAll('#resultsTable tbody tr');
  var visible = 0;
  
  rows.forEach(function(row) {
    if (row.classList.contains('no-data')) return;
    var rowShooter = (row.dataset.shooter || '').toLowerCase();
    var rowTarget = (row.dataset.target || '').toLowerCase();
    var rowMatch = row.dataset.match || '';
    
    var show = true;
    if (shooter && !rowShooter.includes(shooter)) show = false;
    if (target && !rowTarget.includes(target)) show = false;
    if (match && rowMatch !== match) show = false;
...
function sortTable(col) {
  ...
  rows.sort(function(a, b) {
    ...
    if (col === 'score') {
      ...
      aVal = a.dataset.shooter.toLowerCase();
      bVal = b.dataset.shooter.toLowerCase();
    } else if (col === 'target') {
      aVal = a.dataset.target.toLowerCase();
      bVal = b.dataset.target.toLowerCase();
    } else if (col === 'match') {
      aVal = a.dataset.match.toLowerCase();
      bVal = b.dataset.match.toLowerCase();
    }

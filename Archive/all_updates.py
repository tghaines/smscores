...
def upload_scores(comp_route):
    ...
    try:
        ...
        for entry in raw_data:
            ...
            # Parse total like "57-1X" or "60-10X" or "50-3V"
            import re
            m = re.match(r'(\d+)-(\d+)[XV]?', total_str)
            if m:
                score = int(m.group(1))
...
<div class="container">
  ...
  <div class="upload-box" id="dropZone">
    <p style="margin-bottom:16px; color:var(--text2);">Drag and drop a JSON file here</p>
  ...
  <div class="format-help">
    ...
    <pre>[
  {"match":"Match One 10+2", "user":"John Smith", "shots":"6,6,5,X,6,5,5,X,6,6", "total":"57-2X"},
  {"match":"Match Two 10+2", "user":"John Smith", "shots":"X,X,6,6,6,5,5,5,5,6", "total":"56-2X"},
  ...
]</pre>
...
def api_update_shooter(comp_id):
    ...
    db.session.commit()
    return jsonify({'success': True, 'message': f'Updated {old_name} to {new_name or old_name}'})
...
<div class="modal" id="add-shooter-modal">
  <div class="modal-content">
    <div class="modal-header">
      <h2>Add Shooter</h2>
      <button class="modal-close" onclick="closeModal('add-shooter-modal')">x</button>
    </div>
    <div class="form-row">
...
<div class="modal" id="edit-shooter-modal">
  <div class="modal-content">
    <div class="modal-header">
      <h2>Edit Shooter</h2>
      <button class="modal-close" onclick="closeModal('edit-shooter-modal')">x</button>
    </div>
...
<div class="modal" id="add-score-modal">
  <div class="modal-content">
    <div class="modal-header">
      <h2>Add Score</h2>
      <button class="modal-close" onclick="closeModal('add-score-modal')">x</button>
    </div>
    <div class="form-row">
      <div class="form-group">
        <label>Shooter</label>
...
function renderShooters(filter) {
  ...
  tbody.innerHTML = filtered.map(s => 
    ...
    '<td>' + (s.class || '-') + '</td>' +
    '<td>' +
    '<button class="edit-btn" onclick="showEditShooter(\'' + s.name.replace(/'/g, "\\'") + '\', \'' + (s.class || '').replace(/'/g, "\\'") + '\')">Edit</button>' +
    '<button class="delete-btn" onclick="deleteShooter(\'' + s.name.replace(/'/g, "\\'") + '\')">Delete</button>' +
    '</td>' +
    '</tr>'
  ).join('');
}
...
function renderScores(filter) {
  ...
  scores.forEach(shooter => {
    if (filter && shooter.name.toLowerCase().indexOf(filter.toLowerCase()) === -1) return;
    ...
    (shooter.matches || []).forEach(m => {
      ...
      rows.push(
        ...
        '<td><button class="delete-btn" onclick="deleteScore(\'' + shooter.name.replace(/'/g, "\\'") + '\', \'' + m.match.replace(/'/g, "\\'") + '\')">Delete</button></td>' +
        '</tr>'
      );
    });
  });
...
function renderMatches() {
  ...
  container.innerHTML = matches.map(m => 
    ...
    '<input type="text" value="' + m + '" data-original="' + m + '" onchange="renameMatch(\'' + m.replace(/'/g, "\\'") + '\', this.value)">' +
    '</div>' +
    '</div>'
  ).join('');
}
